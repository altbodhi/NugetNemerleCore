...
namespace Nemerle.Tools.MSBuildTask
{
    public class Ncc : ToolTask
    {
        ...
        {
          var ncc_core = Path.Combine(Path.GetDirectoryName(new Uri(typeof(Ncc).Assembly.CodeBase).LocalPath), "ncc-core.dll");
          return base.ExecuteTool(pathToTool, responseFileCommands, $" exec {ncc_core} " + commandLineCommands);
        }
